...
void delay_init(void)
{
	...
	timer_set_prescaler(DELAY_TIMER, rcc_apb1_frequency/1000000-1);
...
{
	for(uint16_t start_cnt=timer_get_counter(DELAY_TIMER); (uint16_t)(timer_get_counter(DELAY_TIMER)-start_cnt) < delay; );
}
